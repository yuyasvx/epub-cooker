...
import { isIncluded } from "../FileExclude";
...
export const documentItemCollector: ItemCollector = {
  ...
  async collect(context, project) {
    ...
    const dirents = await runThrowing(fileIo.getListDetails(dataSourceDir));
...
class Collector {
  ...
  public async collectItem(
    ...
    if (dirent.isDirectory() && isIncluded(directory, dirent.name, undefined, includedRules, excludedRules)) {
      ...
      const newDirents = await runThrowing(fileIo.getListDetails(resolve(rootDirectory, newDirectoryName)));
